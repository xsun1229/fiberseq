...
bin_labels <- paste(seq(10, 130, by = 10), seq(20, 140, by = 10), sep = "-")

df_filtered_samestrand <- df_filtered[df_filtered$same_strand == T,]

df_binned <- df_filtered_samestrand %>%
  mutate(
    bin = cut(
      distance,
      breaks = seq(10, 140, by = 10),
      include.lowest = TRUE,
      right = TRUE,
      labels = bin_labels
    ),
    log_or = log2(FET_oddratio)  # natural log odds ratio
  ) %>%
  filter(!is.na(bin), is.finite(log_or))

# Boxplot of log-OR by bin
ggplot(df_binned, aes(x = bin, y = log_or)) +
  geom_boxplot(fill = "skyblue", alpha = 0.7, outlier.size = 1) +
  theme_minimal(base_size = 14) +
  labs(
    x = "Distance Bin (bp)",
    y = "log2(Odds Ratio)",
    title = "The pairs are on same strand"
  ) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

df_filtered_diffstrand <- df_filtered[df_filtered$same_strand == F,]

df_binned <- df_filtered_diffstrand %>%
  mutate(
    bin = cut(
      distance,
      breaks = seq(10, 140, by = 10),
...
ggplot(df_binned, aes(x = bin, y = log_or)) +
  ...
  labs(
    x = "Distance Bin (bp)",
    y = "log2(Odds Ratio)",
    title = "The pairs are on different strands"
  ) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
