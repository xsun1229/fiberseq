...
samples <- data.table::fread("/project/spott/xsun/fiberseq/data/LCL_sample_metatable_merged_samples.csv",fill = T)

DT::datatable(samples,caption = htmltools::tags$caption( style = 'caption-side: left; text-align: left; color:black;  font-size:150% ;','Samples'),options = list(pageLength = 10) )
...
From the Digital genomic footprinting (DGF) files, they have two LCL lines: https://resources.altius.org/~jvierstra/projects/footprinting.2020/Supplementary_Table_1.xlsx
[3:14 PM]maybe use the GM12865
[3:15 PM]https://resources.altius.org/~jvierstra/projects/footprinting.2020/per.dataset/GM12865-DS12436/
[3:18 PM]You can download those to my directory /project/spott/kevinluo/Fiber_seq/data/Vierstra_DGF


## Motif cluster

motif clusters https://www.vierstra.org/resources/motif_clustering

Kevin has downloaded hg38 motif clusters to /project/spott/kevinluo/Fiber_seq/data/motifs/Vierstra_motif_clustering/

we can use the file `hg38.archetype_motifs.v1.0.bed.gz`

this file contains genome-wide TF motif matches, and they clustered motifs by motif similarity

| Column        | Example        | Description                                     |
|---------------|----------------|-------------------------------------------------|
| contig        | chr1           | Chromosome                                      |
| start         | 10005          | Start position (0-based)                        |
| stop          | 10022          | End position                                    |
| motif_cluster | KLF/SP/2       | Motif cluster name                              |
| match_score   | 3.7536         | MOODS match score for best cluster match        |
| strand        | +              | Matching strand                                 |
| best_model    | RREB1_MA0073.1 | Best matching motif model from cluster          |
| num_models    | 1              | Number of motif model from cluster with a match |
...
## ENCODE ATAC-seq peaks

`/project/spott/kevinluo/Fiber_seq/data/ATACseq`

ATAC-seq peaks in ENCODE have different versions. Default are pseudoreplicated peaks and also IDR peaks that fewer false positivies.
